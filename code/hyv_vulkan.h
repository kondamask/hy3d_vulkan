...
#define VK_LOAD_GLOBAL_FUNCTION(fun)                       \
...
#define VK_LOAD_INSTANCE_FUNCTION(instance, fun)            \
...
#define VK_LOAD_DEVICE_FUNCTION(device, fun)            \
...
static bool Win32InitializeVulkan(vulkan_state &vulkan, HINSTANCE &wndInstance, HWND &wndHandle, const char *name, u32 width, u32 height)
{
    ...
    if (!VkLoadInstanceFunctions(vulkan.instance))
        return false;
    ...
    if (!VkLoadDeviceFunctions(vulkan.device))
        return false;
