...
del *.pdb > NUL 2> NUL
cl  %COMPILER_FLAGS% /I %VULKAN_SDK%\Include ..\code\hyv_engine.cpp -Fmhyv_engine.map -LD -link -incremental:no -opt:ref  -PDB:hyv_engine_%RANDOM%.pdb %EXPOTED_FUNCS%
cl  %COMPILER_FLAGS% ..\code\hyv.cpp -Fmhyv.map -Fe%EXE_NAME% -link %LINKER_FLAGS%
popd
